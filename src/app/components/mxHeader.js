        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-2">
              <Disc className={`w-8 h-8 text-lime-400 ${isScrolled ? '' : 'animate-spin-slow'}`} />
              <span className="text-2xl font-metal text-lime-400">
                mysoreXPress
              </span>
            </div>
            ...
            <nav className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                ...
                >
                  {link.name}
                  <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-lime-400 
                    transition-all group-hover:w-full"></span>
                </a>
              ))}
              <button className="bg-lime-500 hover:bg-lime-600 text-emerald-950 
